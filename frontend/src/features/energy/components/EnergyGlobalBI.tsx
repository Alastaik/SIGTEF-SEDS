...
import type { GlobalEnergyDashboard } from '../types';
import { 
  ComposedChart, Area, Bar, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, 
  PieChart, Pie, Cell 
} from 'recharts';
import { Filter, Building2, Zap, DollarSign, Activity } from 'lucide-react';

interface Props {
  dashboard: GlobalEnergyDashboard;
}
...
export function EnergyGlobalBI({ dashboard }: Props) {
  ...
  const availableEntities = useMemo(() => {
    ...
    dashboard.records.forEach(r => {
      if (r.legalEntityId) {
        entityMap.set(r.legalEntityId, r.legalEntityName || 'Desconhecido');
      }
    });
    return Array.from(entityMap.entries()).map(([id, name]) => ({ id, name })).sort((a, b) => a.name.localeCompare(b.name));
  }, [dashboard.records]);

  const availableUnits = useMemo(() => {
    if (!dashboard.records) return [];
    const units = new Set(dashboard.records.filter(r => 
      (selectedEntity === 'ALL' || r.legalEntityId === selectedEntity) && r.consumerUnitNumber
    ).map(r => r.consumerUnitNumber));
    return Array.from(units) as string[];
  }, [dashboard.records, selectedEntity]);

  const filteredRecords = useMemo(() => {
    if (!dashboard.records) return [];
    return dashboard.records.filter(r => {
      if (selectedEntity !== 'ALL' && r.legalEntityId !== selectedEntity) return false;
      if (selectedUnit !== 'ALL' && r.consumerUnitNumber !== selectedUnit) return false;
      return true;
    });
  }, [dashboard.records, selectedEntity, selectedUnit]);

  // Aggregate by month since global may have multiple records per month
  const chartData = useMemo(() => {
    const monthlyAgg: Record<string, { kwh: number, valor: number }> = {};
    
    filteredRecords.forEach(r => {
      const disp = r.competenceDisplay || 'N/A';
      if (!monthlyAgg[disp]) {
        monthlyAgg[disp] = { kwh: 0, valor: 0 };
      }
      monthlyAgg[disp].kwh += r.kwhAmount;
      monthlyAgg[disp].valor += r.totalValue;
    });
    ...
    const data = Object.entries(monthlyAgg).map(([month, vals]) => {
      const monthPrefix = month.substring(0, 3);
      const custoUnitario = vals.kwh > 0 ? vals.valor / vals.kwh : 0;
      return {
        name: month,
        sortKey: monthOrder[monthPrefix] || 0,
        kwh: vals.kwh,
        valor: vals.valor,
        custoUnitario
      };
  ...
  if (!dashboard.records || dashboard.records.length === 0) {
    return (
      <div className="h-64 flex items-center justify-center text-slate-400 text-sm bg-white rounded-xl border border-slate-200">
        Nenhum dado de energia registrado para os filtros selecionados neste ano.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <select 
            className="text-sm border-slate-200 rounded-lg focus:ring-emerald-500 focus:border-emerald-500 outline-none max-w-[200px]"
            value={selectedEntity}
            onChange={e => {
              setSelectedEntity(e.target.value);
              setSelectedUnit('ALL'); // reset unit when entity changes
            }}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-emerald-950 to-emerald-900 rounded-xl p-6 text-white shadow-md relative overflow-hidden">
          ...
          <p className="text-emerald-300/70 text-xs mt-2">{filteredRecords.length} faturas agregadas</p>
        </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-slate-500 text-sm font-medium">Preço Médio / kWh Global</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
          <h3 className="text-base font-semibold text-slate-800 mb-6">Evolução Global de Custo x Consumo</h3>
          {chartData.length > 0 ? (
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                  ...
                  <YAxis yAxisId="left" tick={{ fontSize: 12, fill: '#64748b' }} tickFormatter={(val) => `R$ ${val >= 1000 ? (val/1000).toFixed(0)+'k' : val}`} axisLine={false} tickLine={false} />
                  <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 12, fill: '#64748b' }} tickFormatter={(val) => `${val >= 1000 ? (val/1000).toFixed(0)+'k' : val} kWh`} axisLine={false} tickLine={false} />
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm flex flex-col">
          ...
          <p className="text-xs text-slate-500 mb-6">Proporção do valor total gasto no ano</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-sm">
        <h3 className="text-base font-semibold text-slate-800 mb-6">Variação do Custo do kWh Médio (R$)</h3>
        {chartData.length > 0 ? (
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={chartData} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
